...
<tr><font color="red" > ${hint}</font> </tr>
		...
		<form id="textform" name="textform"  method="post" action="<%=request.getContextPath()%>/bacl_admin.do?comd=n13_5" >
			...
			<c:if test="${pagei.list != null}">
				<c:forEach items="${pagei.list}" var="VO"
					...
					<input type="checkbox" name="questionCheckbox" value="${VO.teamId}" />
						...
						<td align="center">
							<a href="<%=request.getContextPath()%>/bacl_admin.do?comd=n138&id=${VO.questionID}">重新分配</a> 
						</td>
					</tr>
				</c:forEach>
			</c:if>
			<tr>
				<td align="center" colspan="7" >&nbsp;&nbsp;&nbsp;<input type="button" onclick="batchQuestion()" value="批量分配" /></td>
			</tr>
		</table>
