   ...
   function  updateGoldLoft(itemId){
	...
	<body class="main_body">
		...
		<table width="98%" border="0px" cellpadding="0" cellspacing="0"
			...
			<c:if test="${not empty loftList}">
				<c:forEach var="loft" items="${loftList}" begin="0"
					varStatus="status">
					...
					<c:choose>
						<c:when test="${status.index%2==0}">
							<tr align="center">
						 	...
						 	<td width="15%">
									<fmt:formatDate value="${loft.beginTime}" pattern="yyyy-MM-dd HH:mm" />
									
								</td>
						 
						 	<td width="15%">
									<fmt:formatDate value="${loft.calcTime}" pattern="yyyy-MM-dd HH:mm" />
								</td>
						 	...
						 	<td width="5%">
								...
								<td width="10%">
									<a href="#" onclick="updateGoldLoft('${loft.roomId}')">踢出藏金阁</a>&nbsp;&nbsp;
								</td>
							</tr>
						</c:when>
						<c:otherwise>
		...
		<form id="editForm"
			action="<%=request.getContextPath()%>/bacl_admin.do?comd=updateLoft"
			method="post">
