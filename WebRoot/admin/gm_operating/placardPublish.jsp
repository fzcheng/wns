...
<html>
	...
	<body class="main_body">
	    ...
	    String message = (String)request.getAttribute("message");
	     ...
	     %>
	      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong style="font-size: 30">发布公告</strong>
	      <c:if test="${my:hasPermission(backUser.roleId,50010) || my:hasPermission(backUser.roleId,50009)}">
	      <form id="textform" name="textform" action="#"  method="post">
						...
						<table width="50%" style="margin-left: 60" border="0" cellpadding="0" cellspacing="0">
							...
							<c:if test="${my:hasPermission(backUser.roleId,50011)}">
							<tr>
								<td>立即发布</td>
								<td>
							...
							<tr>
								...
								<c:if test="${plaBean == null}">
									<input type="button" onclick="checkData('<%=request.getContextPath()%>/bacl_admin.do?comd=createPlacard')" VALUE="提交">
								</c:if>	
								<c:if test="${plaBean != null}">
									<input type="button" onclick="checkData('<%=request.getContextPath()%>/bacl_admin.do?comd=updatePlacard')" VALUE="更新">
								</c:if>	
									<input type="button" onclick="javascript:window.location.href='<%=request.getContextPath()%>/bacl_admin.do?comd=n5_2'" VALUE="取消">
								</td>
							</tr>
						</table>
					</form><br/><br/>
					...
					<c:forEach items="${plaList}" var="pla" varStatus="sta">
						<tr> 
							...
							<c:if test="${pla.status == 0}">
								<c:if test="${my:hasPermission(backUser.roleId,50012)}">
								<input type="button" onclick="javascript:window.location.href='<%=request.getContextPath()%>/bacl_admin.do?comd=toPlaTop&id=${pla.id}'" value="置顶">
								</c:if>
								<c:if test="${my:hasPermission(backUser.roleId,50010)}">
								<input type="button" onclick="javascript:window.location.href='<%=request.getContextPath()%>/bacl_admin.do?comd=toEditPla&id=${pla.id}'" value="编辑">
								</c:if>
								<c:if test="${my:hasPermission(backUser.roleId,50014)}">
								<input type="button" onclick="confirmExcute('<%=request.getContextPath()%>/bacl_admin.do?comd=removePlacard&id=${pla.id}','确认将公告下线?')" value="下线">
								</c:if>
							</c:if>
							<c:if test="${pla.status == 1}">
								...
								<c:if test="${my:hasPermission(backUser.roleId,50011)}">
								<input type="button" onclick="confirmExcute('<%=request.getContextPath()%>/bacl_admin.do?comd=publishPlacard&id=${pla.id}','确认发布公告?')" value="发布">
								</c:if>
								<c:if test="${my:hasPermission(backUser.roleId,50013)}">
								<input type="button" onclick="confirmExcute('<%=request.getContextPath()%>/bacl_admin.do?comd=deletePlacard&id=${pla.id}','确认删除公告?')" value="删除">
								</c:if>
							</c:if>
